/* permcheck - checks and reports on access permissions via fts(3). only
 * unix permissions are considered; exotic ACLs or other curiosities
 * (apparmor, selinux) are not checked
 *
 * see also parsepath(1) for a different (and older) take on this task
 *
 * a better approach in most cases is to specify and enforce some
 * permissions policy; however, this tool might help in figuring out
 * what is broken while trying to devise such a policy
 *
 * NOTE access(2) while attractive is not used as it according to the
 * docs by default checks the parent directories for each file; when
 * recursing with fts(3), these should only be checked once. hence the
 * custom permission checking code (and risk of custom bugs) */
...
/* catch-all exit code for "something awry during fts(3) search" (or
 * during the pre-fts(3) parent directory permissions tests) */
...
void emit_help(void);
/* 'amode' is something like R_OK|X_OK (for directories) that is tested
...
int main(int argc, char *argv[])
{
    ...
    /* the username will have been filled in if -u specified, but always
     * need it for the getgrouplist(3) work that in turn is necessary
     * for the permissions checks. well, unless the user is root, in
     * which case the groups do not apply */
    if (Flag_User_ID != 0) {
        if (username == NULL) {
        ...
        /* portability note - getgrouplist(3) on Slackware 3.10.17 uses
         * 'gid_t' while Mac OS X 10.9.5 and OpenBSD 5.5 use 'int' for
         * various group ID fields */
        while (getgrouplist
    ...
    /* user-supplied paths for fts(3) must be made sane (and exist) and
     * parent directories checked for permissions problems, as running
     * fts(3) over /home/jdoe is a waste of time if /home is the source
     * of the problem. */
    ...
    while (*argp) {
        ...
        /* fts(3) should check the starting inode, so we only need to
         * cover directories above that. alas, Linux screws around with
         * the realp, so must make a copy and dirname() that */
    ...
    *plp = (char *) 0;
    ...
    while ((filedat = fts_read(filetree)) != NULL) {
        ...
        case FTS_DC:
            /* a symlink loop lists the symlink twice; ln(1) and link(2)
             * fail to create a hard link on Mac OS X. TODO test on
             * other OS. what is it with OS disallowing hard linked
             * directories? what could possibly go wrong?? */
            if (Flag_Verbose) {
        ...
        case FTS_ERR:
            ...
            /* report should provide more detail than "Permission
             * denied", though do show the errno if it is not */
        ...
        default:
            /* silence a warning; everything relevant should be handled
             * by the above. if not, see fts(3) */
            ;
        }
    }
    if (errno != 0)
...
/* checks the permissions on the given stat, returns 0 if access is
 * allowed, or some positive value on failure (the bits that failed the
 * permissions test); amode is something like R_OK|X_OK that is applied
 * to the user, group, and other, in turn */
unsigned int file_access(struct stat *sb, unsigned int amode, int *ugo)
{
    unsigned int tmode;
    if (Flag_User_ID == 0)
    ...
    /* unix, once gets match on user or group, stops looking at
     * subsequent (Stevens, APUE, ch 4, section 5 (p.81 1st edition;
     * p.94 2nd edition)). this can be confirmed by owning a directory
     * but giving it 0007 modes; the owner should fail read attempts */
    tmode = amode << 6;
