...
main(int argc, char *argv[])
{
	...
	if (Kill_Bill) {
	    if (!Flag_Quiet)
		warnx("duration %ld exceeded: killing pid %d", iTimer.it_value.tv_sec, bill);
	    /*
...
parse_duration(char *durp)
{
    ...
    ret = sscanf(durp, "%d%n", &tmp_duration, &advance);
    if (ret != 1)
	errx(EX_DATAERR, "duration must be positive integer (62) or short form (1m2s)");
    durp += advance;
	...
	while ((ret = sscanf(durp, "%d%1c%n", &tmp_duration, &duration_spec, &advance)) == 2) {
	    durp += advance;
	    sleep_duration += spec2secs(tmp_duration, duration_spec);
	}
...
spec2secs(int d, char c)
{
	...
	break;
    default:
	err(EX_DATAERR, "unknown duration specification '%c', must be one of 'smhdw'", c);
    }
