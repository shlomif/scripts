/* muss-with-environ - experiment with manually changing the environ(7)
 * pointer to include duplicate entries. View the results with e.g.
 *
...
void emit_help(void)
{
    fputs("Usage: muss-with-environ command [args ..]\n", stderr);
    exit(EX_USAGE);
}
